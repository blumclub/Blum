export default async function fetchProducts() {
    try {
        ...
        const productos = csv.split("\n").slice(1).map((row) => {
            const [idProducto, NombreProducto, Descripcion, DescripcionExt, Precio, Imagen ] = row.split(',');
            return { idProducto, NombreProducto, Descripcion, DescripcionExt, Precio, Imagen };
        });
